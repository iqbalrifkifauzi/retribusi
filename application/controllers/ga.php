...
class ga extends CI_Controller {
	...
	public function sptrd_tambah() {
			...
			$data['data_apk'] = $this->ga_model->GetApk();
			$data['data_kecamatan'] = $this->ga_model->GetKecamatan();
	...
	public function sptrd_simpan() {
			...
			// $this->form_validation->set_rules('nomor_kontrak', 'Plat Nomor', 'required');
			...
			if ($this->form_validation->run() == FALSE)
			{
				...
				$cek = $this->ga_model->NomorImbSama($imb);

				if ($cek->num_rows()>0) {

					$this->session->set_flashdata('sama','IMB sama ');
					redirect("ga/sptrd");
						...
						$in_data['retri'] 					= $this->input->post('retri');
						$in_data['kec'] 					= $this->input->post('kec');
						$in_data['nama_perusahaan'] 					= $this->input->post('nama_perusahaan');
						// $in_data['tahun_pembuatan'] 		= $this->input->post('tahun_pembuatan');
						$in_data['jumlah'] 			= $this->input->post('jumlah');
						$in_data['luas_bangunan'] 			= $this->input->post('luas_bangunan');
	...
	public function stnk_perpanjangan_pajak_excel () {
			...
			$data['data_sptrd'] 					= $this->ga_model->GetStnkPerpanjangPajak($bulan2,$bulan1,$bulan);
	...
	public function master_asuransi_tambah() {
			...
			// $data['data_kecamatan_asuransi'] = $this->ga_model->GetApkAsuransi();
	...
	public function asuransi_tambah () {
			...
			$data['data_kecamatan_asuransi'] 		=  $this->ga_model->GetApkAsuransi();
			$data['data_kecamatan_asuransi_view'] 	=  $this->ga_model->GetApkAsuransiView();
	...
	public function asuransi_simpan () {
			...
			$this->form_validation->set_rules('sptrd_id', 'STNK', 'required');
			...
			if ($this->form_validation->run() == FALSE)
			{
				...
				$this->template_dua->load('template_dua','ga/sistem/asuransi/add',$data);
						...
						$in_data['sptrd_id'] 							= $this->input->post('sptrd_id');
	...
	public function ganti_tahun_edit() {
		...
		foreach ($query->result_array() as $value) {
			...
			$data['nama_pemeriksa'] 		= $value['nama_pemeriksa'];
			$data['berlaku_sampai'] 		= $value['berlaku_sampai'];
			$data['nama_apk'] 				= $value['nama_apk'];
			$data['nama_kecamatan'] 		= $value['nama_kecamatan'];
	...
	public function perpanjang_retribusi_edit() {
		...
		$in_data['sptrd_id'] 					= $this->uri->segment(3);
		$in_data['master_login_id'] 			= $this->session->userdata("id_master_login");

		$id['id_sptrd'] 					= $this->uri->segment(3);
		$update['status_perpanjang_retribusi'] 	= "0";

		$this->db->insert("tbl_sptrd_perpanjangan_retribusi",$in_data);
	...
	public function apk() {
		...
		$data['data_apk'] = $this->ga_model->GetApk();
		$this->template_dua->load('template_dua','ga/sistem/apk/index',$data);
	

}

public function apk_tambah() {

	

		$this->template_dua->load('template_dua','ga/sistem/apk/add');
	
}

public function apk_simpan() {
	

		$this->form_validation->set_rules('nama_apk', 'retri', 'required');
		...
		if ($this->form_validation->run() == FALSE)
		{
			$this->template_dua->load('template_dua','ga/sistem/apk/add');
					...
					$in_data['nama_apk'] 	= $this->input->post('nama_apk');
					$in_data['harga'] 	= $this->input->post('harga');
					
					$this->db->insert("tbl_apk",$in_data);
					

				$this->session->set_flashdata('berhasil','APK Berhasil Disimpan');
				redirect("ga/apk");
...
public function apk_edit() {
	$id_apk = $this->uri->segment(3);

	$query  = $this->ga_model->EditApkID($id_apk);

	foreach ($query->result_array() as $value) {
		$data['id_apk']  	= $value['id_apk'];
		$data['nama_apk']  	= $value['nama_apk'];
		$data['harga']  	= $value['harga'];
		
	}

	$this->template_dua->load('template_dua','ga/sistem/apk/edit',$data);
}
...
public function apk_update() {
		...
		$id['id_apk'] =  $this->input->post('id_apk');

		$in_data['nama_apk'] 	= $this->input->post('nama_apk');
		$in_data['harga'] 	= $this->input->post('harga');

		$this->db->update("tbl_apk",$in_data,$id);

		$this->session->set_flashdata('update','APK Berhasil Diupdate');
		redirect("ga/apk");
...
public function apk_delete(){
		...
		$this->ga_model->DeleteApkId($id_apk);

		$this->session->set_flashdata('message','APK Berhasil Dihapus');
		redirect('ga/apk');
	...
	public function kecamatan() {
			...
			$data['data_kecamatan'] = $this->ga_model->GetKecamatan();
			$this->template_dua->load('template_dua','ga/sistem/kecamatan/index',$data);
	...
	public function kecamatan_tambah() {
			...
			$this->template_dua->load('template_dua','ga/sistem/kecamatan/add');
	...
	public function kecamatan_simpan() {
			...
			$this->form_validation->set_rules('nama_kecamatan', 'kec', 'required');
			...
			if ($this->form_validation->run() == FALSE)
			{
				$this->template_dua->load('template_dua','ga/sistem/kecamatan/add');
						...
						$in_data['nama_kecamatan'] 	= $this->input->post('nama_kecamatan');
						
						$this->db->insert("tbl_kecamatan",$in_data);

					$this->session->set_flashdata('berhasil','Kecamatan Berhasil Disimpan');
					redirect("ga/kecamatan");
	...
	public function kecamatan_edit() {
		...
		$query  = $this->ga_model->EditKecamatanID($id_kecamatan);
		...
		$this->template_dua->load('template_dua','ga/sistem/kecamatan/edit',$data);
	}
	...
	public function kecamatan_update() {
			...
			$this->db->update("tbl_kecamatan",$in_data,$id);

			$this->session->set_flashdata('update','Kecamatan Berhasil Diupdate');
			redirect("ga/kecamatan");
	...
	public function kecamatan_delete(){
			...
			$this->ga_model->DeleteKecamatanId($id_kecamatan);
	...
	public function master_jenis_asuransi_simpan() {
			...
			$this->form_validation->set_rules('nama_kecamatan_asuransi', 'Jenis', 'required');
			...
			if ($this->form_validation->run() == FALSE)
			{
				$this->template_dua->load('template_dua','ga/sistem/master_jenis_asuransi/add');
						...
						$in_data['nama_kecamatan_asuransi'] 	= $this->input->post('nama_kecamatan_asuransi');
						
						$this->db->insert("tbl_kecamata_asuransi",$in_data);
	...
	public function master_jenis_asuransi_edit() {
		$id_kecamatan_asuransi = $this->uri->segment(3);

		$query  = $this->ga_model->EditMasterJenisAsuransiID($id_kecamatan_asuransi);

		foreach ($query->result_array() as $value) {
			$data['id_kecamatan_asuransi']  	= $value['id_kecamatan_asuransi'];
			$data['nama_kecamatan_asuransi']  = $value['nama_kecamatan_asuransi'];
	...
	public function master_jenis_asuransi_update() {
			...
			$id['id_kecamatan_asuransi'] =  $this->input->post('id_kecamatan_asuransi');

			$in_data['nama_kecamatan_asuransi'] 	= $this->input->post('nama_kecamatan_asuransi');
			

			$this->db->update("tbl_kecamata_asuransi",$in_data,$id);
	...
	public function master_jenis_asuransi_delete(){
			...
			$this->ga_model->DeleteMasterJenisId($id_kecamatan_asuransi);
	...
	public function laporan_tahun_pembuatan_cek () {
		...
		$data['data_sptrd'] =  $this->ga_model->GetTahunPembuatanTahun($tahun);
	...
	//Awal Laporan 	retribusi sptrd Tahun
	...
	public function laporan_tahun_retribusi_sptrd_excel () {
