...
class ga_model extends CI_Model {
	...
	function EditSptrdId($id_sptrd) {
		return $this->db->query("select a.*,a.alamat as alamat_sptrd,b.*,c.* 
		from tbl_sptrd a 
		left join tbl_apk b on a.retri=b.id_apk
		left join tbl_kecamatan c on a.kec=c.id_kecamatan
		
		where a.id_sptrd='$id_sptrd' ");
	}
	...
	function DetailAsuransiId($id_ga_asuransi) {
		...
		join tbl_apk e on c.type=e.id_apk
...
//awal perpanjang
	...
	function GetApk() {
		return $this->db->query("select * from tbl_apk order by id_apk desc");
	}

	function EditApkID($id_apk) {
		return $this->db->query("select * from tbl_apk where id_apk='$id_apk' ");
	} 

	function DeleteApkId($id_apk) {
		return $this->db->query("delete from tbl_apk where id_apk='$id_apk' ");
	}
...
- (right(curdate(),5) < right(tgl_perolehan,5))+$pilihan) as bulan,c.*,d.* from tbl_ga_stnk_perolehan a 
	...
	function DetailPerolehanId($id_ga_stnk_perolehan) {
		...
		join tbl_ga_stnk c on a.ga_stnk_id=c.id_ga_stnk 
		left join tbl_apk e on c.type=e.id_apk
	...
	function GetTahunPembuatanTahun ($tahun) {
		return $this->db->query("select a.*,b.*,c.*
		 from tbl_sptrd a
		 join tbl_apk b on a.retri=b.id_apk
		 join tbl_kecamatan c on a.kec=c.id_kecamatan
	...
	function GetStatusStnk($status) {
		return $this->db->query("select a.*,b.*,c.*
		 from tbl_ga_stnk a
		 join tbl_apk b on a.type=b.id_apk
